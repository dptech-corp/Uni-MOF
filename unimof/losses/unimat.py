...
class UniMatLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logits_encoder, encoder_distance, encoder_coord, lattice, x_norm, delta_encoder_pair_rep_norm = model(**sample[input_key], encoder_masked_tokens=masked_tokens)
        ...
        if lattice is not None:
            ...
            lattice_loss = F.mse_loss(
                lattice.view(-1, 3).float(),
                lattice_target.view(-1, 3),
                reduction="mean",
            )
